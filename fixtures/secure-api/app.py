"""Secure multi-tenant API fixture.

All tenant isolation vulnerabilities from the vulnerable version are fixed.
This is synthetic test code demonstrating proper authorization boundaries.
"""
import sqlite3
...
def create_app(database=':memory:'):
    ...
    def list_projects():
        db = get_db()
        # FIX: Tenant boundary enforced -- only own tenant's projects
        rows = db.execute(
    ...
    def get_project(pid):
        db = get_db()
        # FIX: Tenant boundary enforced -- only returns projects from caller's tenant
        # Returns 404 (not 403) to prevent resource enumeration
        row = db.execute(
            "SELECT id, tenant_id, name, description, budget, updated_at "
            "FROM projects WHERE id = ? AND tenant_id = ?",
            (pid, g.current_user['tenant_id']),
        ).fetchone()
        if not row:
    ...
    def update_project(pid):
        db = get_db()
        # FIX: Tenant boundary enforced -- only modifies own tenant's projects
        row = db.execute(
    ...
    def list_users():
        db = get_db()
        # FIX: Tenant boundary enforced -- only own tenant's users
        rows = db.execute(
            "SELECT id, tenant_id, name, email, role FROM users WHERE tenant_id = ?",
            (g.current_user['tenant_id'],),
        ).fetchall()
        return jsonify([dict(r) for r in rows])
    ...
    def get_user(uid):
        db = get_db()
        # FIX: Tenant boundary enforced -- only returns users from caller's tenant
        row = db.execute(
            "SELECT id, tenant_id, name, email, role FROM users "
            "WHERE id = ? AND tenant_id = ?",
            (uid, g.current_user['tenant_id']),
        ).fetchone()
        if not row:
            return jsonify({"error": "Not found"}), 404
        return jsonify(dict(row))
...
if __name__ == '__main__':
    ...
    init_db(application)
    print("Secure API running on http://localhost:5002")
    print("Try: curl -H 'X-API-Key: acme-admin-key-001' http://localhost:5002/api/projects/3")
